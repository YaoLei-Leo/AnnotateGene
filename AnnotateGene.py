...
def main(GenomeAssembly, GenomicRegion, Color, Figsize, DotPerInch, Mutation, MutationFile, Output):
    ...
    if MutationFile:
        with open(MutationFile) as f:
            for line1 in f:
                ...
                list1 = line1.split(",")
                if list1[0]=="SNV":
                    ax = AnnotateMutation.AnnotateSNVMutation(ax, int(list1[1]), list1[2], list1[3])
                    # print(list1)
                elif list1[0]=="CNV":
    ...
    ax.set_xlabel(Chr, fontsize=20) ### Set x label
    plt.xticks(rotation = 45) ### Rotate tick angle
...
if __name__=="__main__":
    ...
    parser.add_argument('-c', '--Color', metavar="", type=str, required=False, default="blue,green,blue,blue", help='The color of intron, UTR, exon, and transcript name, respectively. Default is "blue,green,blue,blue". Hexadecimal color codes are supported.')
    ...
    parser.add_argument('-dpi', '--DotPerInch', metavar="", type=int, required=False, default=100, help='The dpi of the figure. default is 100.')
    parser.add_argument('-o', '--Output', metavar="", type=str, default="./PlotGene.png", required=False, help='The output image, default is "./PlotGene.png".')
